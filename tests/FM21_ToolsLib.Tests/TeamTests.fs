...
type TeamTests() =
    ...
    member _.``buildTeam assigns distinct ball playing defenders and sets expected roles`` () =
        ...
        let iwbR = mkPlayer "IWB_R" "D (R)" [ ("Pas",18); ("Tec",17); ("Cro",16); ("Pac",17) ]
        let iwbL = mkPlayer "IWB_L" "D (L)" [ ("Pas",17); ("Tec",16); ("Cro",15); ("Pac",16) ]
        let bpdA = mkPlayer "BPD_A" "D (C)" [ ("Pas",19); ("Tec",18); ("Cmp",17); ("Tck",14) ]
        let bpdB = mkPlayer "BPD_B" "D (C)" [ ("Pas",18); ("Tec",17); ("Cmp",16); ("Tck",15) ]
        let wgr = mkPlayer "WGR" "M (R)" [ ("Cro",19); ("Pac",18); ("Acc",18); ("Dri",15) ]
        let iwl = mkPlayer "IWL" "M (L)" [ ("Pas",18); ("Tec",17); ("Dri",16); ("OtB",15) ]
        let bwm = mkPlayer "BWM" "M (C)" [ ("Tck",19); ("Mar",18); ("Agg",17); ("Sta",16) ]
        let ap  = mkPlayer "AP"  "M (C)" [ ("Pas",10); ("Tec",19); ("OtB",18); ("Cmp",17) ]
        let afa = mkPlayer "AFA" "ST (C)" [ ("Pac",19); ("Acc",18); ("Fin",19); ("Dri",14) ]
        let tma = mkPlayer "TMA" "ST (C)" [ ("Fin",18); ("Pac",17); ("Acc",17); ("Hea",15) ]
    ...
    member _.``teamAsStrings contains human readable role lines and teamScoreOption is Some when all assigned`` () =
        ...
        let iwbR = mkPlayer "IWB_R" "D (R)" [ ("Pas",18); ("Tec",17); ("Cro",16) ]
        let iwbL = mkPlayer "IWB_L" "D (L)" [ ("Pas",17); ("Tec",16); ("Cro",15) ]
        let bpdA = mkPlayer "BPD_A" "D (C)" [ ("Pas",19); ("Tec",18); ("Cmp",17) ]
        let bpdB = mkPlayer "BPD_B" "D (C)" [ ("Pas",18); ("Tec",17); ("Cmp",16) ]
        let wgr = mkPlayer "WGR" "M (R)" [ ("Cro",19); ("Pac",18); ("Acc",18) ]
        let iwl = mkPlayer "IWL" "M (L)" [ ("Pas",18); ("Tec",17); ("Dri",16) ]
        let bwm = mkPlayer "BWM" "M (C)" [ ("Tck",19); ("Mar",18); ("Agg",17) ]
        let ap  = mkPlayer "AP"  "M (C)" [ ("Pas",10); ("Tec",19); ("OtB",18) ]
        let afa = mkPlayer "AFA" "ST (C)" [ ("Pac",19); ("Acc",18); ("Fin",19) ]
        let tma = mkPlayer "TMA" "ST (C)" [ ("Fin",18); ("Pac",17); ("Acc",17) ]
